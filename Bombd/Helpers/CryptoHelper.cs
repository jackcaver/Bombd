...
public static class CryptoHelper
{
    ...
    {
        byte[] outer = new byte[4 + data.Count];
        byte[] inner = new byte[20];

        for (int i = 0; i < 4; ++i)
        {
            byte b = (byte)(salt >> ((3 - i) * 8));
            outer[i] = (byte)(b ^ 0x5c);
            inner[i] = (byte)(b ^ 0x36);
        }

        Buffer.BlockCopy(data.Array!, data.Offset, outer, 4, data.Count);
        byte[] hash = MD5.HashData(outer);
        Buffer.BlockCopy(hash, 0, inner, 4, hash.Length);

        return MD5.HashData(inner);
    }

    public static ushort GetMD516(ArraySegment<byte> data, uint salt)
    {
        byte[] hmac = GetHMAC(data, salt);
        ushort result = 0;
    ...
    public static int GetMD532(ArraySegment<byte> data, uint salt)
    {
        byte[] hmac = GetHMAC(data, salt);
        uint result = 0;
    ...
    public static string GetRandomUUID() => Guid.NewGuid().ToString();
